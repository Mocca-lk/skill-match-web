...
export function eibirMensagemStatus(status, txtMessage, container) {
    if (status === "carregando") {
        ...
        container.innerHTML = `<p class="status-msg erro">❌ ${mensagemTexto}</p>`   
     }
}
...
export function renderizarDestaque(dadosDestaque, totalAnalises, container) {
    ...
    <div class="card-destaque">
      ...
      <p><strong>${melhorVaga.vaga.cargo}</strong> na empresa <strong>${melhor.vaga.empresa}</strong> (${melhor.analise.porcentagem}% de match)</p>
